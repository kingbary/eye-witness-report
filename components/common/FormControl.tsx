...
import { FieldValues, Path, RegisterOptions, UseFormRegister, FieldError } from 'react-hook-form';
...
type ControlProps = (
    ...
    labelText?: string;
    radius?: string;
    error?: string | FieldError;  // Update to accept FieldError
    register?: Register;
        ...
        content = (
            <div className="relative">
                <input
                    ...
                    className={`${error ? "border-red-600" : "border-[#CBD5E1]"} border rounded-md h-12 outline-none px-4 text-neutral-dark-2 w-full pr-10`} // Added padding to the right for the icon
                />
                {props.type === 'password' && (
                    <span
    ...
    return (
        ...
        >
            ...
            {getErrorMessage(error) && (
                <span className="text-red-600 text-xs">{getErrorMessage(error)}</span>
            )}
            {/* {icon} */}
        </div>
    );
